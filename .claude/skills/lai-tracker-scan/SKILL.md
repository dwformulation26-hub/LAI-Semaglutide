...
- **Every text field the dashboard shows gets a Korean copy, written in the same edit.** Finding `summary` → `summary_ko`; `current_status` `stage`, `stage_evidence`, `molecule_evidence`, `dosing_target`, `partner` and `data_point` → the same name plus `_ko`; candidate evidence `snippet` → `snippet_ko`; `promotion_bar.evidence` → `promotion_bar.evidence_ko`. English stays the source of truth: write the English, check it against the source, then translate the English — never write the Korean from the source directly, even a Korean source. Follow [references/ko-glossary.md](references/ko-glossary.md) for spellings and copy every number and development code exactly; `scripts/build.mjs` fails otherwise. Whenever a run overwrites an English field, it rewrites that field's Korean copy in the same edit. Names (`canonical_name`, `detected_name`), aliases, source names and `verification_note` stay English only.
...
|---|---|
| `"send"` | The run logged at least one **new finding**, or wrote at least one **new candidate** to `candidates`. Send the digest to the standing recipient list, in the exact order the routine prompt lists it. |
| `"draft"` | Everything else — a digest carrying only repeated escalations or late items. Create the Gmail draft and stop. The admin reads that one and sends it by hand, or doesn't. |

This is a standing authorization the admin granted on 2026-09-16 (extended to new candidates on 2026-09-18), and it is deliberately narrow: new findings and new candidates are news and go out unattended; a queue that merely hasn't moved (a repeated escalation, an old late item) is not, and never mails anyone on its own. A candidate only reaches `candidates` at all after clearing `named_entity` + `technical_claim` + a Tier 1/2 source (see the candidate pipeline above), which is a high enough bar to justify the same unattended send as a finding. Do not re-judge the `delivery` value against your own read of how interesting the run was, in either direction — the whole point of deciding it in the renderer is that the rule doesn't drift day to day. Sending remains the only unattended outward action in this skill: the promotion boundary above is untouched, and creating, promoting, merging, rejecting or snoozing anything is still admin-only, always.
...
**Known incident, twice now (2026-09-20, 2026-09-22): the value that actually reached the recipients was not the rendered digest.** Once it was the literal shell-style text `$(cat email/draft-output.json | html)` — a run wrote a command instead of the file's content, and the Gmail tool has no shell and sends exactly the string it's given. Once it was the literal word `PLACEHOLDER`. A third run sent the real content but hand-retyped it into the tool call and mangled the first line into a visible, escaped `&lt;!doctype html&gt;`. All three are the same failure: composing the `html` argument by re-typing or summarizing it in the conversation instead of carrying the file's bytes through unchanged. Each one required an emergency follow-up (a corrected resend or a draft) and, on a `"send"`, already reached the standing recipient list once with unusable content before anyone caught it.

**So, mechanically, every single time:**
- Get `html` and `subject` onto the clipboard for the tool call by reading `email/draft-output.json` directly in the same step that builds the Gmail call — never from memory of what the file "should" contain, never as a shell command, a file path, or a stand-in word, however tempting when the value is long.
- Immediately after the `create_draft` or `send_message` call returns, fetch what you just created (`get_thread` on the returned `threadId`, or re-read the draft) and check its reported size against `email/draft-output.json`'s `html` length (`wc -c`) — they should be within a few hundred bytes of each other (HTML entity expansion accounts for the rest). The three broken sends above were 900-950 bytes; every real digest on this tracker has run 11,000-18,000 bytes. A send under ~2KB is not a real digest — treat it exactly as a failed send under the rule below, even though the tool call itself returned success.
- Only report the email step done, in either the run summary or `meta.json`, after that size check passes. A tool call returning an ID is not delivery confirmation for this step; a plausible byte count is.

**4. Deliver it per `delivery`.** On `"draft"`, create a Gmail draft and stop. On `"send"`, send it to the routine prompt's recipient list in the order given, and say in your final summary that you sent it and to how many people. Either way the mail is the *last* thing the run does, after the commit is on `main`: a failure here must never cost the run its data. If the Gmail tool fails on a `"send"`, or the post-send size check above fails, fall back to leaving a draft and say so plainly — do not retry a send you cannot confirm, since the failure mode is four people getting the same digest twice.
...
- Never compose the Gmail tool's `html`/`subject` values by re-typing, summarizing, or shell-referencing `email/draft-output.json` — read the file and carry its values through unchanged, then verify the sent/drafted message's size against it before calling the email step done (see the Email digest section's "Known incident" note; this has already shipped unusable content to the recipient list twice).
