...
Last synced with the live routines on 2026-09-14.
...
Email digest: follow the skill's 'Email digest' section exactly. Build email/draft-input.json with findings, lateItems, candidates and the six-row coverage array (one row per class in the fixed order, day counts in KST calendar days, computed from the umbrella files after this run's edits). Every finding and late item carries `molecules` copied from that record's molecule_class, and `stageChange` only when this finding moved stage_label. The leadIn is a one-sentence news headline: no internal terms ('umbrella', 'sweep', 'wire skim', coverage fractions) and no admin-queue wording. Run `node scripts/render-email.mjs email/draft-input.json email/draft-output.json`, and only if it writes an output file, pass its subject and html to the Gmail draft tool unmodified (draft only, never send). If there is nothing to report, or node or the script errors, skip the email and say so -- the commit is the part that must not fail.
```
...
Update data/meta.json's last_run.weekly_sweep and last_run.qc_tier2, coverage (coverage.weekly_sweep, coverage.qc_tier2, coverage.candidate_follow_up) and source_health even if nothing new is found. Edit JSON with targeted Edit calls.
...
Email digest: identical rules to the Daily Scan -- findings, lateItems, candidates, the six-row coverage array in KST day counts, `molecules` and `stageChange` per item, a one-sentence news-headline leadIn with no internal or admin-queue wording, render with `node scripts/render-email.mjs email/draft-input.json email/draft-output.json`, and pass the output to the Gmail draft tool unmodified (draft only, never send).
```
