...
Last synced with the live routines on 2026-09-14 (second update the same day: no sub-agents, blocked-host rule, stamp helper, escalations array).
...
| Connector | Gmail (draft only, never send) |
...
Email digest: follow the skill's 'Email digest' section exactly. Build email/draft-input.json with findings, escalations (every unresolved ready_for_promotion candidate, with newlyEscalated: true only on the run it escalated), lateItems (each with its confidence), candidates (only those created this run) and the six-row coverage array (one row per class in the fixed order, day counts in KST calendar days, computed from the umbrella files after this run's edits). Every finding and late item carries `molecules` copied from that record's molecule_class, and `stageChange` only when this finding moved stage_label. The leadIn is a one-sentence news headline: no internal terms ('umbrella', 'sweep', 'wire skim', coverage fractions) and no admin-queue wording. Run `node scripts/render-email.mjs email/draft-input.json email/draft-output.json`, and only if it writes an output file, pass its subject and html to the Gmail draft tool unmodified (draft only, never send). If the script rejects the leadIn, rewrite the sentence and run it again. If there is nothing to report, or node or the script fails for any other reason, skip the email and say so -- the commit is the part that must not fail.
```
...
This routine fires Sunday 09:00 KST, the only run between Friday's and Monday's daily scans. Check data/meta.json's last_run.daily_scan first: if it is more than 12 hours before now (always true on the scheduled Sunday run), run Track A and Track B1 exactly as a Daily scan would, first, and write last_run.daily_scan and coverage.daily_scan as well. Only skip them when a daily scan ran within the last 12 hours.

Then run 'Weekly sweep' mode exactly as the skill's mode table defines it: Track B2, Track B3's weekly deep pass, and QC Tier 2. Track B2 is class- and code-aware: the per-molecule clinicaltrials.gov and Google Patents queries through domain-scoped search, one query for every untracked-program code in the watchlist, code harvesting from every result, plus the structural sources (conference accepted-abstract indices including ADA, EASD and ObesityWeek, KIPRIS/USPTO/WIPO depot and sustained-release CPC classes, the KOSDAQ 기술특례상장 pipeline, university tech-transfer, VC/deal databases). Patents never advance stage_label and are logged with their priority date. Keep Track A's breadth-first pass ahead of every B2 query.
...
Update data/meta.json's last_run.weekly_sweep and last_run.qc_tier2, coverage (coverage.weekly_sweep, coverage.qc_tier2, coverage.candidate_follow_up) and source_health even if nothing new is found. Write findings, candidates and meta.json with targeted Edit calls, and stamp dates with `node scripts/stamp.mjs checked <KST date> <umbrella-id>...` (umbrellas Track A queried) and `node scripts/stamp.mjs followup <KST date> <candidate-id>...` (candidates the deep pass re-searched). Summaries, snippets and source names describe the world only; how an item was found or checked goes in verification_note.
...
Email digest: identical rules to the Daily Scan -- findings, escalations (every unresolved ready_for_promotion candidate; Sunday digests repeat them), lateItems with confidence, candidates created this run, the six-row coverage array in KST day counts, `molecules` and `stageChange` per item, a one-sentence news-headline leadIn with no internal or admin-queue wording, render with `node scripts/render-email.mjs email/draft-input.json email/draft-output.json` (if it rejects the leadIn, rewrite the sentence and render again), and pass the output to the Gmail draft tool unmodified (draft only, never send).
```
