...
Before committing, run `node scripts/build.mjs` and `npm test`. Both must pass. If the build rejects a stage_label, molecule_class, molecule_evidence, molecule_stages, a finding's molecules or a candidate status, fix the data value -- never the script, the tests, the template, the watchlist or any UI code.
...
QC Tier 2: of the 5-10 findings sampled for re-verification, include at least one record from each named class that has records (tirzepatide, retatrutide, amylin), and for those records also confirm molecule_class still matches molecule_evidence, and that each molecule_stages entry still matches what that molecule's own named assets are evidenced at -- a stage that drifted onto a molecule from a sibling asset reads as clinical progress the sources never reported, and checking each finding against its source cannot catch it. Record a mismatch in meta.json's qc_tier2 notes rather than silently changing the class or the stage. Then run the fuzzy-dedup sweep across the alias table, normalizing codes (case, hyphens, spaces) before comparing.
